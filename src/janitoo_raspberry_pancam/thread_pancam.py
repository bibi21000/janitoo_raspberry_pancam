...
def make_thread(options, force=False):
    if get_option_autostart(options, OID) or force:
        return PancamThread(options)
    else:
        return None

class PancamThread(JNTBusThread):
    """The basic thread
    ...
    def init_bus(self):
        """Build the bus
        """
        from janitoo_raspberry_pancam.pancam import PancamBus
        self.section = OID
        self.bus = PancamBus(options=self.options, oid=self.section, product_name="Raspberry pancam controller")
